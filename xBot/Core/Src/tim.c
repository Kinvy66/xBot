...
void HAL_TIM_Encoder_MspDeInit(TIM_HandleTypeDef* tim_encoderHandle)
{
  ...
  /* USER CODE END TIM3_MspDeInit 0 */
    ...
    PA7     ------> TIM3_CH2
    */
    HAL_GPIO_DeInit(GPIOA, ENC_A1_Pin|ENC_B1_Pin);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance==TIM4)  // 20ms
  {
    ...
    read_encoder(&encoder1,&encoder2);
    ...
    tb6612_pwm_output(pwm1,pwm2);

    mcu_data.encoder1 = encoder1;
    mcu_data.encoder2 = encoder2;

    //语音识别ID读取
    if(asr_new_recv_flag)
    {
      mcu_data.asr_id = asr_id;
      asr_new_recv_flag = 0;
    }
    else
    {
